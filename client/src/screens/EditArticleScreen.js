...
const EditArticleScreen = () => {
  ...
  useEffect(() => {
    ...
    if (articleDetails.coverImage) {
      setImage(articleDetails.coverImage);
    }
    if (articleDetails.content) {
      setContent(articleDetails.content);
    }
    if (articleDetails.excerpt) {
      setExcerpt(articleDetails.excerpt);
    }
  }, [
    userInfo,
    navigate,
